...
/**
 * WaveNet (Cefas Data Hub) → public JSON for a single buoy (auto-detected Cardigan).
 * Outputs:
 *  - public/latest.json
 *  - public/history.json
 ...
 *  - CEFAS_RECORDSET_ID = 12651   (WaveNet 48h recordset)
 *  - INST_ID            = 353     (optional override; numeric Instrument Id)
 */
...
const PREFERRED_INST = (process.env.INST_ID || "").replace(/\D/g, ""); // e.g., "353"

const norm = (s) => String(s ?? "").trim();
const lc = (s) => norm(s).toLowerCase();

// Parse "15/08/2025 07:30:00" or ISO-ish strings to UTC ISO
function toISO(dt) {
  const s = norm(dt);
  // D/M/Y H:M[:S]
  const m1 = s.match(
...
if (!rows.length) {
  ...
  fs.writeFileSync(
    "public/diagnostics.json",
    JSON.stringify({ note: "CSV empty" }, null, 2)
  );
  process.exit(0);
}

// Column detection (based on your diagnostics)
const headers = Object.keys(rows[0]);
const pickExact = (names) => headers.find((h) => names.includes(h));
const pickLoose = (needles) =>
  headers.find((h) => needles.some((n) => lc(h).includes(lc(n))));
...
const instCol =
  pickExact(["InstId", "InstID", "InstrumentID", "InstrumentId"]) ||
  pickLoose(["inst", "instrument"]);
const depCol =
  pickExact(["Deployment", "DeploymentName"]) ||
  pickLoose(["deploy", "location", "site", "name"]);
const paramCol =
  pickExact(["Parameter", "ParameterCode", "ParamCode"]) ||
  pickLoose(["parameter", "param", "variable", "observed", "property", "name"]);
const valueCol =
  pickExact(["ResultMean", "Value", "Result", "Reading", "DataValue", "NumericValue"]) ||
  pickLoose(["value", "result", "reading"]);

// Summarise instruments present (InstId + Deployment)
...
const stations = [...stationMap.values()]
  .map((s) => ({ ...s, params: [...s.params] }))
  .sort((a, b) => b.rowCount - a.rowCount);

// Pick Cardigan instrument
function pickStation() {
  const cardigan = stations.filter((s) =>
    lc(s.deployment || "").includes("cardigan")
  );
  if (cardigan.length) return cardigan[0];
  if (PREFERRED_INST) {
    const byInst = stations.filter((s) => (s.instId || "") === PREFERRED_INST);
    if (byInst.length) return byInst[0];
  }
  return stations[0]; // fallback
}
const chosen = pickStation();

// Build time series for chosen instrument (pivot long → wide)
const byTs = new Map();
...
function pickField(raw, patterns) {
  ...
  for (const re of patterns) {
    const hit = keys.find((k) => re.test(canon(k)));
...
const series = [...byTs.values()]
  .map((e) => {
    const obj = {
      ...
      hm0: pickField(
        e.raw,
        mk(["^hm0$", "significantwaveheight", "^hs$", "hm0m"])
      ),
      ...
      temp: pickField(e.raw, mk(["^temp$", "seatemperature", "watertemperature"])),
      spread: pickField(
        e.raw,
        mk(["^w_spr$", "^wspr$", "directionalspread", "^spr$"])
      ),
      raw: e.raw, // keep raw mapping for transparency/debug
    };
    // Belt-and-braces: if dir still missing but exact "W_PDIR" exists
  ...
  JSON.stringify(
    {
      ...
      paramCol,
      valueCol,
      preferredInst: PREFERRED_INST || null,
      chosen,
      paramLabelSamples: Array.from(labelSet).slice(0, 40),
      countTimestamps: series.length,
      latestSummary: latest
        ? { ts: latest.ts, keys: Object.keys(latest.raw || {}) }
        : null,
    },
    null,
    2
...
console.log(`Built ${series.length} records; chosen station:`, chosen);
